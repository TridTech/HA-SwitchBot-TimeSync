...
### Configuring Time Offset (If Needed)

If your device shows incorrect time after syncing (e.g., 2 hours off):

1. Go to **Settings** → **Devices & Services**
2. Find **SwitchBot Meter Time Sync**
3. Click on your device
4. Click **Configure** (gear icon)
5. Set **Time offset (hours)**:
   - Use **negative** values if device is ahead (e.g., `-2`)
   - Use **positive** values if device is behind (e.g., `+2`)
   - Use **0** for no offset (default)
6. Click **Submit**
7. Test by pressing sync button

**Each device can have its own offset!**

See [CONFIGURE_OFFSET.md](CONFIGURE_OFFSET.md) for detailed guide.
...
**Issue**: Your SwitchBot Meter doesn't appear in the device list.

**Quick Fixes**:
1. **Active Scanning**: Go to Settings → Devices & Services → Bluetooth, ensure adapter is set to "Active" mode
2. **Pairing Mode**: Press and hold the button on your Meter for 2-3 seconds until Bluetooth icon appears
3. **Move Closer**: Place device within 1-3 meters of your Bluetooth adapter
4. **Remove from Official Integration**: If already added to SwitchBot Bluetooth integration, temporarily remove it

**See TROUBLESHOOTING.md for detailed help**

### Device Already in Official Integration

The device can be in BOTH integrations simultaneously:
- Official integration: Provides temperature/humidity sensors
- This integration: Provides time sync button

They work together without conflicts.

### Debug Your Device

Run the included debug script to see what your device is advertising:
...
This will show you:
- Whether your device is advertising
- What data it's sending  
